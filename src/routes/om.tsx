...
import { sanityClient, aboutPageQuery, type AboutPage as AboutPageData } from "@/lib/sanity";
...
export const Route = createFileRoute("/om")({
  ...
  head: () => ({
    meta: [
      { title: "Om butikken – BJ Wine" },
      { name: "description", content: "Læs om BJ Wine og vores tilgang til vin." },
      { property: "og:title", content: "Om butikken – BJ Wine" },
      { property: "og:description", content: "Læs om BJ Wine og vores tilgang til vin." },
  ...
  return (
    <section className="container-editorial py-20">
      ...
      <h1 className="font-display text-5xl leading-tight text-primary sm:text-6xl">
        ...
        {accent && <span className="block italic text-brass">{accent}</span>}
      </h1>
      <div className="mt-12 grid gap-12 md:grid-cols-[2fr_1fr]">
        <div className="prose-editorial space-y-6 text-lg leading-relaxed text-foreground/85">
          {data?.body ? <PortableText value={data.body} /> : <p>Tilføj indhold i CMS'et.</p>}
        </div>
        {(asideTitle || asideBody) && (
          <aside className="border border-border/70 bg-paper p-8">
            {asideTitle && <p className="eyebrow mb-3">{asideTitle}</p>}
            {asideBody && (
              <p className="text-sm leading-relaxed text-muted-foreground">{asideBody}</p>
            )}
          </aside>
        )}
      </div>
